...
exports.getHeadersList = function(localDomain, getForward, res, callback) {
    Domain.findOne({localDomain: localDomain}, (getForward) ? "forwardHeaders" : "returnHeaders", function(err, currentSite) {
        if (currentSite) {
            callback(res, err, (getForward) ? results.forwardHeaders : results.returnHeaders, getForward);
        } else {
...
exports.storeData = function(localDomain, hash, path, headers, json, isProtected, callback) {
    Domain.findOne({localDomain: localDomain}, "_id", function(err, currentSite) {
        if (currentSite) {
            ...
            Data.findOne({domainId: currentSite._id, hash: hash}, function(err, results) {
                if (results) {
                    let newData = (typeof isProtected === "boolean") ? {headers: headers, json: json, isProtected: isProtected} : {headers: headers, json: json};
                    ...
                    let newData = {domainId: currentSite._id, hash: hash, path: path, headers: headers, json: json, isProtected: isProtected === true};
