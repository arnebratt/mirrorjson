    ...
    Domain.findOne({localDomain: localDomain}, function(err, results) {
        if (results) {
            Domain.update({localDomain: localDomain}, {$set: {remoteDomain: remoteDomain}}, function (err, numberAffected) {
                if (!err) {
                    Data.deleteMany({domainId: results._id}, function (err, result) {
                        console.log(result ? (result.deletedCount + " documents related to '" + remoteDomain + "' deleted.") : err);
                        callback(false);
                    })
                } else {
                    console.log(err);
    ...
    Domain.findOne({localDomain: localDomain}, "_id", function(err, currentSite) {
        if (currentSite) {
            ...
            Data.findOne({domainId: currentSite._id, hash: hash}, function(err, results) {
                if (results) {
                    let newData = (typeof isProtected === "boolean") ? {headers: headers, json: json, isProtected: isProtected} : {json: json};
