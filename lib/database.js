...
// Update json for given hash
exports.updateData = function(localDomain, hash, json, callback) {
    Domain.findOne({localDomain: localDomain}, "_id", function(err, currentSite) {
        if (currentSite) {
            Data.update({hash: hash}, {$set: {json: json}}, function (err, numberAffected) {
...
exports.storeData = function(localDomain, hash, path, json, callback) {
    Domain.findOne({localDomain: localDomain}, "_id", function(err, currentSite) {
        if (currentSite) {
            if (!hash) {
                hash = md5(path);
            ...
            Data.findOne({domainId: currentSite._id, hash: hash}, function(err, results) {
                if (results) {
                    exports.updateData(localDomain, hash, json, callback);
                } else {
                    Data.create({domainId: currentSite._id, hash: hash, path: path, json: json}, function(err, data) {
        ...
        if (currentSite) {
            ...
            Data.findOne({domainId: currentSite._id, hash: hash}, function(err, results) {
                if (results) {
                    ...
                    console.log(err);
                    return res.send("Error: Requested data not found for hash " + hash)
                }
            });
        } else {
