    ...
    Domain.findOne({localDomain: localDomain}, "_id", function(err, currentSite) {
        if (currentSite) {
            ...
            Data.findOne({domainId: currentSite._id, hash: hash}, function(err, results) {
                if (results) {
                    let newData = (typeof isProtected === "boolean") ? {statusCode: statusCode, headers: headers, json: json, isProtected: isProtected, lastAccessed: Date.now()} : {statusCode: statusCode, headers: headers, json: json, lastAccessed: Date.now()};
...
exports.getElementMatchPath = function(localDomain, path, res, callback) {
    Domain.findOne({localDomain: localDomain}, "_id", function(err, currentSite) {
        if (currentSite) {
            Data.findOne({domainId: currentSite._id, path: {"$regex": path}}, function(err, results) {
